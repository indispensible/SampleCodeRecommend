...
class TFIDFOperation:
    ...
    def get_tf_idf_score_for_sentences(self, sentence, target_sentence: list):
        sentence_word_list = [item for item in sentence.split(" ") if item not in self.stop_words]
        bow = self.dct.doc2bow(sentence_word_list)
        vector = self.tfidf_model[bow]
        sims = self.similarities_index[vector]
        ...
        for i in range(0, len(target_sentence)):
            for item in sims:
                if self.sentence_list[item[0]][0] == target_sentence[i]:
...
if __name__ == "__main__":
    ...
    score_list = op.get_tf_idf_score_for_sentences('how to always round up to the next integer', [
